...
//    private var bannerHostingVC: UIHostingController<AdvertisementCarouselView>?
    ...
    override func viewDidLoad() {
        ...
        if L102Language.currentAppleLanguage() == "ar" {
            ...
            advertisementRTLLayout.minimumLineSpacing = 0
            advertisementRTLLayout.minimumInteritemSpacing = 0
...
//        bannerHostingVC?.view.removeFromSuperview()
//        bannerHostingVC?.removeFromParent()
//        
//        self.advertisementPage.numberOfPages = self.advertisementVM.arrayOfBanners.count
//        self.advertisementPage.currentPage = 0
//        
//        var carouselView = AdvertisementCarouselView(banners: advertisementVM.arrayOfBanners)
//        carouselView.onPageChange = { [weak self] page in
//            self?.advertisementPage.currentPage = page
//        }
//        
//        let hostingVC = UIHostingController(rootView: carouselView)
//        bannerHostingVC = hostingVC
//        
//        addChild(hostingVC)
//        
//        // Replace advertisementCollection with this view
//        // Make sure advertisementCollection is hidden or removed
//        advertisementCollection.isHidden = true
//        advertisementPage.isHidden = false
//        
//        hostingVC.view.translatesAutoresizingMaskIntoConstraints = false
//        view.addSubview(hostingVC.view)
//        
//        NSLayoutConstraint.activate([
//            hostingVC.view.topAnchor.constraint(equalTo: advertisementCollection.topAnchor),
//            hostingVC.view.leadingAnchor.constraint(equalTo: advertisementCollection.leadingAnchor),
//            hostingVC.view.trailingAnchor.constraint(equalTo: advertisementCollection.trailingAnchor),
//            hostingVC.view.bottomAnchor.constraint(equalTo: advertisementCollection.bottomAnchor)
//        ])
//        
...
//    }
    ...
    private func scrollToNextAdvertisement() {
        ...
        currentAdvertisementPage =
            (currentAdvertisementPage + 1) % totalPages

        advertisementPage.currentPage = currentAdvertisementPage

        scrollAdvertisement (
            to: currentAdvertisementPage,
            animated: true
        )
    }
    
    private func scrollAdvertisement(to page: Int, animated: Bool) {

        let itemWidth = advertisementCollection.frame.width
        guard itemWidth > 0 else { return }

        // ✅ With forceRightToLeft, offset 0 is always the first item (Right in RTL, Left in LTR)
        // Offset increases in the natural reading direction.
        let targetOffsetX = CGFloat(page) * itemWidth

        advertisementCollection.setContentOffset (
            CGPoint(x: targetOffsetX, y: 0),
            animated: animated
        )
    }
    ...
    private func requestBanners() {
        ...
        advertisementVM.fetchedSuccessfully = { [weak self] in
            DispatchQueue.main.async {
                ...
                DispatchQueue.main.async {
                    self.currentAdvertisementPage = 0
                    self.advertisementPage.currentPage = 0

                    self.scrollAdvertisement(to: 0, animated: false)
                }
            }
        }
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == service_CollectionVw {
            ...
            let itemWidth = scrollView.frame.width
             ...
             advertisementPage.currentPage = clampedPage
             currentAdvertisementPage = clampedPage
        }
    }
}
